...
const getVendorOverview = asyncHandler(async (req, res) => {
    ...
    return res.status(200).json(new ApiResponse(200, 'Vendor dashboard fetched successfully', {
...
const getTopItems = asyncHandler(async (req, res) => {
    ...
    return res.status(200).json(new ApiResponse(200, 'Top seller items are displayed', topSellers))
});
...
const orderStatusBreakdown = asyncHandler(async (req, res) => {
    ...
    return res.status(200).json(new ApiResponse(200, 'Order Stats fetched successfully', orderStatus))
});
...
const averageOrderValue = asyncHandler(async (req, res) => {
    ...
    return res.status(200).json(new ApiResponse(200, 'Average order value fetched', averageOrderValue))
});
...
const updateStock = asyncHandler(async (req, res) => {
    ...
    await menu.save()
    return res.status(200).json(new ApiResponse(200, 'Stock updated successfully'))
});
...
const getAllMenu = asyncHandler(async (req, res) => {
    ...
    if (menuItems.length === 0) {
        return res.status(200).json(new ApiResponse(200, 'No menu items to show', menuItems))
    }

    return res.status(200).json(new ApiResponse(200, 'Menu Items fetched successfully', menuItems))
});
...
const lowStockItems = asyncHandler(async(req,res)=>{
    ...
    const lowStockMenu = await menuModel.find({
        restaurant:restaurant._id,
        $expr:{
    ...
    if(lowStockMenu.length === 0){
        return res.status(200).json(new ApiResponse(200,'No items in low stock',lowStockMenu))
    }

    return res.status(200).json(new ApiResponse(200,'Items in low stock fetched successfully',lowStockMenu))
...
const bulkUpload = asyncHandler(async (req, res) => {
    ...
    if (files.length !== items.length) {
        throw new ApiError(
            400,
            "Image count and item count must match"
        );
    }
    ...
    items.forEach((item) => {
        ...
        if (item.mrp < item.price) {
            throw new ApiError(
                400,
                `MRP cannot be less than price for ${item.name}`
            );
        }
    });
    ...
    return res.status(201).json(
        new ApiResponse(
            201,
            "Menu items uploaded successfully",
            {
